...
def save_image(url):
    if url.startswith("file://"):
        ...
        dst_path = os.path.join(f"{HOME_DIR}/eww/player/spotify_cache", "album_art.png")
        shutil.copy(file_path, dst_path)
        return True
    response = requests.get(url)
    if response.status_code == 200:
        with open(f"{HOME_DIR}/eww/player/spotify_cache/album_art.png", "wb") as f:
...
def get_palette():
    ...
    image_file = f"{HOME_DIR}/eww/player/spotify_cache/album_art.png"
...
def on_metadata(*args):
    ...
    palette = get_palette()
    os.system(
        f"convert -modulate 95,65 {HOME_DIR}/eww/player/spotify_cache/album_art.png {HOME_DIR}/eww/player/spotify_cache/album_art_modified.png"
    )
    metadata_new = {
